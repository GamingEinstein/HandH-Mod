...
namespace HandHmod.NPCs.Arcani
{
    [AutoloadBossHead]
    public class Arcani : ModNPC
    {
        ...
        private void spinToWin(Vector2 moveTo, Player p)
        {
            ...
            if (npc.ai[1] > 240)
            {
                ...
                if (npc.ai[1] % 10 == 0 && Main.netMode != 1)
                {
                    ...
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float)Math.Cos(fireAngle9) * 12f, (float)Math.Sin(fireAngle9) * 15f, mod.ProjectileType("Hev"), 100, 3f, Main.myPlayer);
        ...
        private void berrabeam(Player p)
        {
            ...
            if (Main.netMode != 1)
            {
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y, shootToX, shootToY, ProjectileID.CultistBossLightningOrb, 150, 3f, Main.myPlayer);
        ...
        private void hellSwarmMissiles(Player p)
        {
            ...
            if (npc.ai[1] % 8 == 0)
            {
                ...
                shootToX *= speed;
                shootToY *= speed;
                Projectile.NewProjectile(npc.Center.X + fOne, npc.Center.Y + fTwo, shootToX, shootToY, ProjectileID.CultistBossIceMist, 100, 6f, Main.myPlayer, p.whoAmI, 0f);
                Main.PlaySound(2, (int)npc.position.X, (int)npc.position.Y, 11);
            }
        ...
        public override void NPCLoot()
        {
            ...
            {
                Item.NewItem(npc.getRect(), ModContent.ItemType<ArcaniTrophy>());
            }
            if (Main.expertMode)
            {
                npc.DropBossBags();
            ...
            else
            {
                ...
                {
                    Item.NewItem(npc.getRect(), ModContent.ItemType<Items.Weapons.Magic.ArcaStaff>());
                }
                {
                    Item.NewItem(npc.getRect(), ModContent.ItemType<Items.Misc.Materials.CursedCore>(), 4);
                }
                Item.NewItem(npc.getRect(), ModContent.ItemType<Items.Misc.Materials.BloodOfTheHeavenDemigod>(), 30);
            }
        }
