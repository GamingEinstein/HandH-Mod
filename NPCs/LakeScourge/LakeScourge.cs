    ...
    public class LakeScourge : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            if (HandHmodWorld.downedMightOfTheUnderworld)
            {
                ...
                npc.damage = (int)(npc.damage * 100f);
            }
            ...
            if (captiveType != 4)
            {
                ...
                if (attackCool <= 0f && Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    if (captiveType != 3 || jungleAI != -5 && jungleAI != 1)
                    {
                        ...
                        Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 5f * (float)Math.Cos(npc.rotation), speed * (float)Math.Sin(npc.rotation), ModContent.ProjectileType<LakeSphere>(), damage, 300f, Main.myPlayer, GetDebuff(), GetDebuffTime());
