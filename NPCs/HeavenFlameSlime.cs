...
namespace HandHmod.NPCs
{
	public class HeavenFlameSlime : ModNPC
	{
		...
		{
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("HeavenFlameOre"), 2);
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("HeavenlySoul"), 2);
		}
	}
}