...
namespace HandHmod.NPCs.Devitrius
{
    [AutoloadBossHead]
    public class Devitrius : ModNPC
    {
        ...
        private int defaultDefense = 2750;
        ...
        private void flammenWheel(Player p)
        {
            ...
            if (Main.netMode != 1)
            {
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float)Math.Cos(fireAngle1) * 12f, (float)Math.Sin(fireAngle1) * 15f, mod.ProjectileType("Hev"), 100, 3f, Main.myPlayer);
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float)Math.Cos(fireAngle2) * 12f, (float)Math.Sin(fireAngle2) * 15f, mod.ProjectileType("Hev"), 100, 3f, Main.myPlayer);
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float)Math.Cos(fireAngle3) * 12f, (float)Math.Sin(fireAngle3) * 15f, mod.ProjectileType("Hev"), 100, 3f, Main.myPlayer);
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float)Math.Cos(fireAngle4) * 12f, (float)Math.Sin(fireAngle4) * 15f, mod.ProjectileType("Hev"), 100, 3f, Main.myPlayer);
        ...
        private void spinToWin(Vector2 moveTo, Player p)
        {
            ...
            if (npc.ai[1] > 240)
            {
                ...
                if (npc.ai[1] % 10 == 0 && Main.netMode != 1)
                {
                    ...
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float)Math.Cos(fireAngle5) * 12f, (float)Math.Sin(fireAngle5) * 15f, mod.ProjectileType("Hev"), 100, 3f, Main.myPlayer);

                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float)Math.Cos(fireAngle7) * 12f, (float)Math.Sin(fireAngle7) * 15f, mod.ProjectileType("Hev"), 100, 3f, Main.myPlayer);

                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float)Math.Cos(fireAngle9) * 12f, (float)Math.Sin(fireAngle9) * 15f, mod.ProjectileType("Hevw"), 100, 3f, Main.myPlayer);
        ...
        private void berrabeam(Player p)
        {
            ...
            if (Main.netMode != 1)
            {
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y, shootToX, shootToY, ProjectileID.CultistBossFireBall, 100, 3f, Main.myPlayer);
        ...
        private void hellSwarmMissiles(Player p)
        {
            ...
            if (npc.ai[1] % 8 == 0)
            {
                ...
                shootToX *= speed;
                shootToY *= speed;
                Projectile.NewProjectile(npc.Center.X + fOne, npc.Center.Y + fTwo, shootToX, shootToY, ProjectileID.CultistBossFireBall, 150, 6f, Main.myPlayer, p.whoAmI, 0f);
                Main.PlaySound(2, (int)npc.position.X, (int)npc.position.Y, 11);
            }
        ...
        public override void NPCLoot()
        {
            ...
            {
                Item.NewItem(npc.getRect(), ModContent.ItemType<DevitriusTrophy>());
            }
            if (Main.expertMode)
            {
                npc.DropBossBags();
            ...
            else
            {
                ...
                {
                    Item.NewItem(npc.getRect(), ModContent.ItemType<Items.Weapons.Magic.DeviStaff>());
                }
                {
                    Item.NewItem(npc.getRect(), ModContent.ItemType<Items.Misc.Materials.CursedCore>(), 4);
                }
                Item.NewItem(npc.getRect(), ModContent.ItemType<Items.Misc.Materials.BloodOfTheHellDemigod>(), 30);
            }
        }
        ...
        public override void BossLoot(ref string name, ref int potionType)
        {
            ...
            if (!HandHmodWorld.downedDemigod)
            {
                HandHmodWorld.downedDemigod = true;
