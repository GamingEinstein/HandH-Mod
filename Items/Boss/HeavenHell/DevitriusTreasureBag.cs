...
{
    public class DevitriusTreasureBag : ModItem
    {
        public override int BossBagNPC => mod.NPCType("Devitrius");
        ...
        public override void OpenBossBag(Player player)
        {
            ...
            if (Main.rand.NextBool(1))
            {
                ...
                player.QuickSpawnItem(ItemType<BloodOfTheHellDemigod>(), Main.rand.Next(10, 40));
                player.QuickSpawnItem(ItemType<CursedCore>(), Main.rand.Next(2, 4));
                if (Main.rand.Next(7) == 0)
                {
                    player.QuickSpawnItem(ItemType<DeviStaff>());
                }
                if (Main.rand.NextBool(100))
                {
                    if (Main.rand.Next(7) == 0)
                    {
                        ...
                        if (Main.rand.Next(7) == 0)
                        {
                            ...
                            {
                                player.QuickSpawnItem(ItemType<DevitriusTreasureBag>());
                            }
                        }
                    }
                }
